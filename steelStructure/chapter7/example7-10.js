...
const example = (
    ...
    N = 60, // mm, 국부항복강도를 구해야 할 때 사용한다. 예제 7-9 참고.
    반력 = 200, // kN, 국부항복강도를 구해야 할 때 사용한다. 예제 7-9 참고.
    반력_재단_사이_거리 = 200 // 반력과 재단 사이 거리는 188P 그림 7.27과 같이 확인한다. 예제 7-10 참고.
) => {
    ...
    if (반력_재단_사이_거리 >= H / 2) {
        ...
        if (N / H < 0.2) {
            ...
            log(`${(φl * Rn) / 1000}이 반력 = ${반력}kN보다 크면 안전하다.`); // 웨브의 크립플링에 대하여
        }
    }
};
example();

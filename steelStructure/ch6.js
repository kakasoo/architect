...
const example = (
    ...
    Ly = 20 * 12 * 25.4,
    Ix = 166 * 25.4 ** 4, // mm^4
    Lx = 20 * 12 * 25.4, // mm, 부재의 길이

    Ag = 27.5 * 25.4 ** 2,

    rx = 111.25 * 25.4 ** 3, // mm, 단면2차반경 테이블 참고
    ry = 33.3 * 25.4 ** 3, // mm
    r = 0, // rounded. 위의 r과 다른 개념.

    Pu = 1600, // 소요강도. 소요강도가 주어지지 않으면 아랫값들을 대입해야 한다.
    DL = 0, // 고정하중
    LL = 0, // 활화중
    LrL = 0, // 지붕의 활화중
    WL = 0, // 풍하중
    ...
    // log(`여기서 구하는 Fcr은 휨좌굴강도입니다. 앞의 탄성좌굴강도와 다릅니다.`);
    log(`4) 설계압축 강도 산정`);
    ...
    if (slenderness < 4.71 * Math.sqrt(210000 / Fy)) {
        log(`세장비가 4.71 * Math.sqrt(E / Fy)보다 작습니다. 따라서,`);
    ...
    if (!Pu) {
        // 주어진 소요 강도가 없다면 직접 구해야 한다.
        case1 = 1.4 * (DL + FL);
        case2 =
            1.2 * (DL + FL + TL) +
            1.6 * (LL + HL) +
            0.5 * Math.max(LrL, SL, RL);
        case3 = 1.2 * DL + 1.6 * Math.max(LrL, SL, RL) + Math.max(LL, RL);
        case4 = 1.2 * DL + 1.3 * WL + LL + 0.5 * Math.max(LrL, SL, RL);
        case5 = 1.2 * DL + EL + LL + 0.2 * SL;
        case6 = 0.9 * DL + 1.3 * WL + 1.6 * HL;
        case7 = 0.9 * DL + EL + 1.6 * HL;
        Pu = Math.max(case1, case2, case3, case4, case5, case6, case7, Pu);
    }
    ...
    if (Pu < (0.9 * Pn) / 1000) {
        log(`${Pu}KN가 ${(0.9 * Pn) / 1000}KN 보다 작다.`);
